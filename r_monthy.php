...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <?php
            $query = "
            SELECT stat, amount_paid, SUM(amount_paid) AS totol, DATE_FORMAT(dttran, '%M-%Y') AS dttran
            FROM csop_rep_import
            WHERE stat='A' and dttran BETWEEN '2020-01-01' and '2021-01-31' 
            GROUP BY DATE_FORMAT(dttran, '%m%')
            ORDER BY DATE_FORMAT(dttran, '%Y-%m-%d') 
            ";
